...
app.post("/cancel-booking", async (req, res) => {
  try {
    const { payment_intent_id, deposit_intent_id, booking_id, user_email, equipment_title, partial_refund } = req.body;

    const refunds = [];

    // Refund rental payment (partial if within 24hrs)
    if (payment_intent_id) {
      try {
        const pi = await stripe.paymentIntents.retrieve(payment_intent_id);
        const refundAmount = partial_refund
          ? Math.round(pi.amount_received * 0.5)  // 50% refund
          : undefined;  // full refund
        const refund = await stripe.refunds.create({
          payment_intent: payment_intent_id,
          ...(refundAmount ? { amount: refundAmount } : {}),
        });
        refunds.push({ type: 'rental', refund_id: refund.id, status: refund.status });
      } catch (e) {
        console.error('Rental refund error:', e.message);
      }
    }
    ...
    if (deposit_intent_id) {
      try {
        const refund = await stripe.refunds.create({ payment_intent: deposit_intent_id });
        refunds.push({ type: 'deposit', refund_id: refund.id, status: refund.status });
      } catch (e) {
        console.error('Deposit refund error:', e.message);
      }
    }

    // Send cancellation email
    if (user_email && equipment_title) {
      await sendEmail({
        to: user_email,
        subject: `Booking cancelled — ${equipment_title}`,
        html: bookingCancelledEmail({ equipmentTitle: equipment_title, bookingId: booking_id }),
      });
    }

    res.json({ success: true, refunds, booking_id });
  } catch (err) {
